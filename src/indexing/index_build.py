...
from src.indexing.ingest_web import ingest_web  # ✅ updated import
...
# ----------------------------
def _index_chunks(chunks: List, docs_count: int, artifact_name: str) -> IndexStats:
    cfg = Settings.load()
    logger = get_logger()
    ...
    ensure_collection(client, cfg.qdrant_collection, vector_size=3072)
    ...
    logger.info(
        f"Docs: {docs_count} | chunks total: {len(chunks)} | missing: {len(missing_chunks)}"
    )
    ...
    upserted = upsert_chunks(client, cfg.qdrant_collection, missing_chunks, embeddings)
...
def build_pdf_chunks(pdf_dir, cfg: Settings) -> Tuple[List, List]:
    ...
    all_chunks = []
    for d in docs:
        meta = {"title": d.title, "program_id": d.meta.get("program_id")}
        all_chunks.extend(
            chunk_text(
                source="pdf",
                doc_id=d.doc_id,
...
def index_pdfs_bachelor() -> IndexStats:
    cfg = Settings.load()
    docs, chunks = build_pdf_chunks(cfg.pdf_bachelor_dir, cfg)
    return _index_chunks(chunks, docs_count=len(docs), artifact_name="index_pdfs_bachelor.json")


def index_pdfs_master() -> IndexStats:
    cfg = Settings.load()
    docs, chunks = build_pdf_chunks(cfg.pdf_masters_dir, cfg)
    return _index_chunks(chunks, docs_count=len(docs), artifact_name="index_pdfs_master.json")


# ----------------------------
# Excel indexing (Bachelor/Master)
# ----------------------------
def index_excel_bachelors() -> IndexStats:
    cfg = Settings.load()
    chunks = ingest_excel_as_program_chunks(cfg.bachelors_excel, excel_scope="bachelor")
    docs_count = len({c.doc_id for c in chunks})
    return _index_chunks(chunks, docs_count=docs_count, artifact_name="index_excel_bachelors.json")


def index_excel_masters() -> IndexStats:
    cfg = Settings.load()
    chunks = ingest_excel_as_program_chunks(cfg.masters_excel, excel_scope="master")
    docs_count = len({c.doc_id for c in chunks})
    return _index_chunks(chunks, docs_count=docs_count, artifact_name="index_excel_masters.json")


def index_excels_all() -> IndexStats:
    """
    ...
    chunks = b + m
    docs_count = len({c.doc_id for c in chunks})
    return _index_chunks(chunks, docs_count=docs_count, artifact_name="index_excel_all.json")
...
def index_web() -> IndexStats:
    ...
    return _index_chunks(chunks, docs_count=len(docs), artifact_name="index_web.json")
